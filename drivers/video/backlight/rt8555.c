...
#define MAX_BRIGHTNESS_RT8555                    0x7D			// Linear BLED Brightness Control - 83%
	...
	if (level != 0) {
		...
		if (dev->blmap) {
			if (level < dev->blmap_size) {
				...
				rt8555_write_reg(client, 0x06, cal_value);
			} else
		...
		rt8555_write_reg(client, 0x06, 0x00);
	}
		...
		struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		...
		rt8555_write_reg(client, 0x06, level);
	} else {
		rt8555_write_reg(client, 0x04, 0x00);
		rt8555_write_reg(client, 0x06, 0x00);
	}
...
void rt8555_backlight_on(int level)
{
	if (backlight_status == BL_OFF) {
		...
		rt8555_write_reg(main_rt8555_dev->client, 0x00, 0x23);
		rt8555_write_reg(main_rt8555_dev->client, 0x01, 0x06); //8bit mode |OVP 25V |Current Limit 1.5A
		rt8555_write_reg(main_rt8555_dev->client, 0x02, 0x29); //LED Current 20.04mA
	}
